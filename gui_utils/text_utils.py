...
def _get_array_impl(
    ...
    *,
    ...
    lines = [
        np.array(line, dtype=np.uint8).reshape([line.size[1], line.size[0]]) for line in lines
    ]
    ...
    lines = [
        np.pad(line, ((0, line_spacing), (0, 0)), mode="constant") for line in lines[:-1]
    ] + lines[-1:]
    mask = np.concatenate(lines, axis=0)
    alpha = mask
    if outline > 0:
        mask = np.pad(
            mask, int(np.ceil(outline * outline_pad)), mode="constant", constant_values=0
        )
...
def _get_array_impl_dropshadow(
    ...
    **kwargs,
    ...
    mask = np.pad(
        mask, 2 * radius + max(abs(offset_x), abs(offset_y)), mode="constant", constant_values=0
    )
    alpha = mask.astype(np.float32) / 255
