...
_texture_formats = {
    ("uint8", 1): dnnlib.EasyDict(
        type=gl.GL_UNSIGNED_BYTE, format=gl.GL_LUMINANCE, internalformat=gl.GL_LUMINANCE8
    ),
    ...
    ("uint8", 3): dnnlib.EasyDict(
        type=gl.GL_UNSIGNED_BYTE, format=gl.GL_RGB, internalformat=gl.GL_RGB8
    ),
    ("uint8", 4): dnnlib.EasyDict(
        type=gl.GL_UNSIGNED_BYTE, format=gl.GL_RGBA, internalformat=gl.GL_RGBA8
    ),
    ...
    ("float32", 3): dnnlib.EasyDict(
        type=gl.GL_FLOAT, format=gl.GL_RGB, internalformat=gl.GL_RGB32F
    ),
    ("float32", 4): dnnlib.EasyDict(
        type=gl.GL_FLOAT, format=gl.GL_RGBA, internalformat=gl.GL_RGBA32F
    ),
}
...
def read_pixels(width, height, *, pos=0, dtype="uint8", channels=3):
    ...
    gl.glReadPixels(
        int(np.round(pos[0])), int(np.round(pos[1])), width, height, fmt.format, fmt.type, image
    )
    gl.glPopClientAttrib()
...
def draw_rect(*, pos=0, pos2=None, size=None, align=0, rint=False, color=1, alpha=1, rounding=0):
    ...
    size = (
        size
        if size is not None
        else pos2 - pos if pos2 is not None else np.array([1, 1], dtype="float32")
    )
    pos = pos - size * align
    if rint:
        pos = np.rint(pos)
...
def draw_circle(*, center=0, radius=100, hole=0, color=1, alpha=1):
    ...
    draw_shape(
        vertices, mode=gl.GL_TRIANGLE_STRIP, pos=center, size=radius, color=color, alpha=alpha
    )
