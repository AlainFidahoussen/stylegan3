...
class VGG16Features(torch.nn.Module):
    ...
    def __init__(self, device, use_relu=False):
        ...
        self.avgpool = (
            vgg16.avgpool
        )  # TODO: more work can be done to partition any part of the model, but not my jam
...
class VGG16FeaturesNVIDIA(torch.nn.Module):
    ...
    def get_layers_features(
        self, x: torch.Tensor, layers: List[str], normed: bool = False, sqrt_normed: bool = False
    ):
        """
        ...
        features_dict["conv1_2"] = self.conv1_2(
            features_dict["conv1_1"]
        )  # [1, 64, 256, 256] => [1, 64, 256, 256]
        features_dict["pool1"] = self.pool1(
            features_dict["conv1_2"]
        )  # [1, 64, 256, 256] -> [1, 64, 128, 128]

        features_dict["conv2_1"] = self.conv2_1(
            features_dict["pool1"]
        )  # [1, 64, 128, 128] => [1, 128, 128, 128]
        features_dict["conv2_2"] = self.conv2_2(
            features_dict["conv2_1"]
        )  # [1, 128, 128, 128] => [1, 128, 128, 128]
        features_dict["pool2"] = self.pool2(
            features_dict["conv2_2"]
        )  # [1, 128, 128, 128] -> [1, 128, 64, 64]

        features_dict["conv3_1"] = self.conv3_1(
            features_dict["pool2"]
        )  # [1, 128, 64, 64] => [1, 256, 64, 64]
        features_dict["conv3_2"] = self.conv3_2(
            features_dict["conv3_1"]
        )  # [1, 256, 64, 64] => [1, 256, 64, 64]
        features_dict["conv3_3"] = self.conv3_3(
            features_dict["conv3_2"]
        )  # [1, 256, 64, 64] => [1, 256, 64, 64]
        features_dict["pool3"] = self.pool3(
            features_dict["conv3_3"]
        )  # [1, 256, 64, 64] -> [1, 256, 32, 32]

        features_dict["conv4_1"] = self.conv4_1(
            features_dict["pool3"]
        )  # [1, 256, 32, 32] => [1, 512, 32, 32]
        features_dict["conv4_2"] = self.conv4_2(
            features_dict["conv4_1"]
        )  # [1, 512, 32, 32] => [1, 512, 32, 32]
        features_dict["conv4_3"] = self.conv4_3(
            features_dict["conv4_2"]
        )  # [1, 512, 32, 32] => [1, 512, 32, 32]
        features_dict["pool4"] = self.pool4(
            features_dict["conv4_3"]
        )  # [1, 512, 32, 32] -> [1, 512, 16, 16]

        features_dict["conv5_1"] = self.conv5_1(
            features_dict["pool4"]
        )  # [1, 512, 16, 16] => [1, 512, 16, 16]
        features_dict["conv5_2"] = self.conv5_2(
            features_dict["conv5_1"]
        )  # [1, 512, 16, 16] => [1, 512, 16, 16]
        features_dict["conv5_3"] = self.conv5_3(
            features_dict["conv5_2"]
        )  # [1, 512, 16, 16] => [1, 512, 16, 16]
        features_dict["pool5"] = self.pool5(
            features_dict["conv5_3"]
        )  # [1, 512, 16, 16] -> [1, 512, 8, 8]

        features_dict["adavgpool"] = self.adavgpool(
            features_dict["pool5"]
        )  # [1, 512, 8, 8] ~> [1, 512, 7, 7]
        features_dict["fc1"] = self.fc1(
            features_dict["adavgpool"]
        )  # [1, 512, 7, 7] ->> [1, 4096]; w/ReLU
        features_dict["fc2"] = self.fc2(features_dict["fc1"])  # [1, 4096] ->> [1, 4096]; w/ReLU
        features_dict["fc3"] = self.softmax(
            self.fc3(features_dict["fc2"])
        ...
        for layer in layers:
            if normed:
                ...
                result_list.append(
                    features_dict[layer]
                    / torch.tensor(torch.numel(features_dict[layer]), dtype=torch.float).sqrt()
                )
            else:
...
class DiscriminatorFeatures(torch.nn.Module):
    def __init__(self, D):
        ...
        setattr(
            self, "adavgpool", nn.AdaptiveAvgPool2d(4)
        )  # Necessary if images are of different resolution than D.img_resolution
    ...
    def get_layers_features(
        ...
        *** Legend: => conv2d, -> flatten, ->> fc layer, ~> mbstd layer, +> adaptive average pool ***
        ...
        features_dict = (
            OrderedDict()
        )  # Can just be a dictionary, but I plan to use the order of the features later on
        features_dict["from_rgb"] = getattr(self, "from_rgb")(
            x
        )  # [1, 3, D.img_resolution, D.img_resolution] =>
        ...
        for idx, res in enumerate(self.block_resolutions):
            ...
            features_dict[f"b{res}_conv1"] = getattr(self, f"b{res}_conv1")(
                features_dict[f"b{res}_conv0"], gain=np.sqrt(0.5)
            )
            features_dict[f"b{res}_conv1"] = features_dict[f"b{res}_skip"].add_(
                features_dict[f"b{res}_conv1"]
            )

        # Irrespective of the image size/model size, the last block will be the same:
        features_dict["b4_mbstd"] = getattr(self, "b4_mbstd")(
            features_dict["b8_conv1"]
        )  # [1, 512, 4, 4] ~> [1, 513, 4, 4]
        features_dict["b4_conv"] = getattr(self, "b4_conv")(
            features_dict["b4_mbstd"]
        )  # [1, 513, 4, 4] => [1, 512, 4, 4]
        features_dict["b4_conv"] = getattr(self, "adavgpool")(
            features_dict["b4_conv"]
        )  # [1, 512, 4, 4] +> [1, 512, 4, 4]  (Needed if x's resolution is not D.img_resolution)
        features_dict["fc"] = getattr(self, "fc")(
            features_dict["b4_conv"].flatten(1)
        )  # [1, 512, 4, 4] -> [1, 8192] ->> [1, 512]
        ...
        for layer in layers:
            if channels is not None:
                ...
                channels = [
                    c for c in channels if c < max_channels
                ]  # Remove channels that are too high
                ...
                if layer not in ["fc", "out"]:
                    features_dict[layer] = features_dict[layer][
                        :, channels, :, :
                    ]  # [1, max_channels, size, size] => [1, len(channels), size, size]
                else:
                    features_dict[layer] = features_dict[layer][
                        :, channels
                    ]  # [1, max_channels] => [1, len(channels)]
