...
def project(
    ...
    *,
    ...
    torch.Tensor, dict
]:  # output shape: [num_steps, C, 512], C depending on resolution of G
    """
    ...
    if project_in_wplus:  # Thanks to @pbaylies for a clean way on how to do this
        ...
        if start_wavg:
            ...
            print(f"Starting from a random vector (seed: {projection_seed})...")
            z = np.random.RandomState(projection_seed).randn(1, G.z_dim)
            w_avg = G.mapping(torch.from_numpy(z).to(device), None)[
                :, :1, :
            ]  # [1, 1, C]; fake w_avg
            w_avg = G.mapping.w_avg + truncation_psi * (w_avg - G.mapping.w_avg)
    ...
    if loss_paper == "sgan2":
        ...
        target_features = vgg16_features.get_layers_features(
            target, layers, normed=normed, sqrt_normed=sqrt_normed
        )
        ...
        ssim_out = (
            SSIM()
        )  # can be used as a loss; recommended usage: ssim_loss = 1 - ssim_out(img1, img2)
        ...
        target_features = disc.get_layers_features(
            target, layers, normed=normed, sqrt_normed=sqrt_normed
        )
        mse = torch.nn.MSELoss(reduction="mean")
        ssim_out = SSIM()
        ...
        )  # TODO: let user decide which model to use (use list given by clip.available_models()
    ...
    optimizer = torch.optim.Adam(
        [w_opt] + list(noise_buffs.values()), betas=(0.9, 0.999), lr=initial_learning_rate
    )
    ...
    for step in range(num_steps):
        ...
        if loss_paper == "sgan2":
            ...
            synth_features = vgg16_features.get_layers_features(
                synth_images, layers, normed=normed, sqrt_normed=sqrt_normed
            )
            ...
            # Pixel-level MSE
            mse_error = mse(synth_images, target) / (
                G.img_channels * G.img_resolution * G.img_resolution
            )
            ssim_loss = ssim_out(
                target, synth_images
            )  # tracking SSIM (can also be added the total loss)
            ...
            n_digits = int(np.log10(num_steps)) + 1 if num_steps > 0 else 1
            message = (
                f"step {step + 1:{n_digits}d}/{num_steps}: percept loss {percept_error.item():.7e} | "
                f"pixel mse {mse_error.item():.7e} | ssim {ssim_loss.item():.7e} | loss {loss.item():.7e}"
            )
            print(message, end="\r")

            last_status = {
                "percept_error": percept_error.item(),
            ...
            synth_features = disc.get_layers_features(
                synth_images, layers, normed=normed, sqrt_normed=sqrt_normed
            )
            percept_error = sum(map(lambda x, y: mse(x, y), target_features, synth_features))
    ...
    return (
        w_out.repeat([1, G.mapping.num_ws, 1]),
        run_config,
    )  # [num_steps, 1, C] => [num_steps, L, C]
...
@click.option(
    "--cfg",
    help="Config of the network, used only if you want to use one of the models that are in torch_utils.gen_utils.resume_specs",
    type=click.Choice(["stylegan2", "stylegan3-t", "stylegan3-r"]),
)
@click.option(
    "--target",
    "-t",
...
@click.option(
    ...
    is_flag=True,
    help="Add flag to stabilize the latent space/anchor to w_avg, making it easier to project (only for StyleGAN3 config-r/t models)",
)
# Video options
@click.option(
    "--save-video", "-video", is_flag=True, help="Save an mp4 video of optimization progress"
)
@click.option(
    "--compress",
    is_flag=True,
...
@click.option(
    ...
    "-wavg",
    type=bool,
    help="Start with the average W vector, ootherwise will start from a random seed (provided by user)",
    default=True,
    show_default=True,
)
@click.option(
...
@click.option(
    "--loss-paper",
    "-loss",
    type=click.Choice(["sgan2", "im2sgan", "discriminator", "clip"]),
    help='Loss to use (if using "im2sgan", make sure to norm the VGG16 features)',
    default="sgan2",
    show_default=True,
)
...
@click.option(
    ...
    "normed",
    is_flag=True,
    help="Add flag to norm the VGG16 features by the number of elements per layer that was used",
)
@click.option(
    "--vgg-sqrt-normed",
    "sqrt_normed",
    is_flag=True,
    help="Add flag to norm the VGG16 features by the square root of the number of elements per layer that was used",
)
...
@click.option(
    ...
    "-saveall",
    is_flag=True,
    help="Save every step taken in the projection (save both the dlatent as a.npy and its respective image).",
)
@click.option(
    "--outdir",
...
def run_projection(
    ...
    if start_wavg:
        ...
        result_name, npy_name = (
            f"{result_name}_seed-{projection_seed}",
            f"{npy_name}_seed-{projection_seed}",
        )
    ...
    if save_every_step:
        ...
        for step in tqdm(range(num_steps), desc="Saving projection results", unit="steps"):
            ...
            PIL.Image.fromarray(synth_image, "RGB").save(
                f"{result_name}_step{step:0{n_digits}d}.jpg"
            )
    ...
    if save_video:
        video = imageio.get_writer(
            f"{result_name}.mp4", mode="I", fps=fps, codec="libx264", bitrate="16M"
        )
        ...
        for projected_w in projected_w_steps:
            ...
            )  # left side target, right projection
        video.close()
    ...
    if save_video and compress:
        ...
        gen_utils.compress_video(
            original_video=f"{result_name}.mp4",
