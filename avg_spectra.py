...
def stream_source_images(
    source, num, seed, device, data_loader_kwargs=None
):  # => num_images, image_size, image_iter
    ...
    if ext == "pkl":
        ...
        raise click.ClickException(
            "--source must point to network pickle, dataset zip, or directory"
        )
...
@click.option(
    "--source",
    help="Network pkl, dataset zip, or directory",
    metavar="[PKL|ZIP|DIR]",
    required=True,
)
@click.option(
...
def stats(source, num, seed, device=torch.device("cuda")):
    ...
    num_images, _image_size, image_iter = stream_source_images(
        source=source, num=num, seed=seed, device=device
    )
...
@click.option(
    "--mean", help="Dataset mean for whitening", metavar="FLOAT", type=float, required=True
)
@click.option(
    "--std",
...
def calc(source, dest, mean, std, num, seed, beta, interp, device=torch.device("cuda")):
    ...
    torch.multiprocessing.set_start_method("spawn")
    num_images, image_size, image_iter = stream_source_images(
        source=source, num=num, seed=seed, device=device
    )
