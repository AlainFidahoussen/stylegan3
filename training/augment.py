...
class AugmentPipe(torch.nn.Module):
    def __init__(
        ...
        for i in range(1, Hz_fbank.shape[0]):
            ...
            Hz_fbank[
                i, (Hz_fbank.shape[1] - Hz_hi2.size) // 2 : (Hz_fbank.shape[1] + Hz_hi2.size) // 2
            ] += Hz_hi2
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.yflip > 0:
            i = torch.floor(torch.rand([batch_size], device=device) * 2)
            i = torch.where(
                torch.rand([batch_size], device=device) < self.yflip * self.p,
                i,
                torch.zeros_like(i),
            )
            if debug_percentile is not None:
        ...
        if self.scale > 0:
            ...
            if debug_percentile is not None:
                s = torch.full_like(
                    s, torch.exp2(torch.erfinv(debug_percentile * 2 - 1) * self.scale_std)
                )
        ...
        if self.rotate > 0:
            ...
            theta = torch.where(
                torch.rand([batch_size], device=device) < p_rot, theta, torch.zeros_like(theta)
            )
            if debug_percentile is not None:
        ...
        if self.aniso > 0:
            ...
            if debug_percentile is not None:
                s = torch.full_like(
                    s, torch.exp2(torch.erfinv(debug_percentile * 2 - 1) * self.aniso_std)
                )
        ...
        if self.contrast > 0:
            ...
            if debug_percentile is not None:
                c = torch.full_like(
                    c, torch.exp2(torch.erfinv(debug_percentile * 2 - 1) * self.contrast_std)
                )
            C = scale3d(c, c, c) @ C
        ...
        if self.saturation > 0 and num_channels > 1:
            ...
            if debug_percentile is not None:
                s = torch.full_like(
                    s, torch.exp2(torch.erfinv(debug_percentile * 2 - 1) * self.saturation_std)
                )
                ...
                if debug_percentile is not None:
                    t_i = (
                        torch.full_like(
                            t_i,
                            torch.exp2(
                                torch.erfinv(debug_percentile * 2 - 1) * self.imgfilter_std
                            ),
                        )
                        if band_strength > 0
        ...
        if self.noise > 0:
            ...
            images = (
                images
                + torch.randn([batch_size, num_channels, height, width], device=device) * sigma
            )
