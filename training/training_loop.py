...
def training_loop(
    ...
    if rank == 0:
        ...
        save_image_grid(
            images, os.path.join(run_dir, "reals.png"), drange=[0, 255], grid_size=grid_size
        )
        ...
        save_image_grid(
            images, os.path.join(run_dir, "fakes_init.png"), drange=[-1, 1], grid_size=grid_size
        )
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function("data_fetch"):
            ...
            all_gen_z = [
                phase_gen_z.split(batch_gpu) for phase_gen_z in all_gen_z.split(batch_size)
            ]
            all_gen_c = [
            ...
            all_gen_c = [
                phase_gen_c.split(batch_gpu) for phase_gen_c in all_gen_c.split(batch_size)
            ]
