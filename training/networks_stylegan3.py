...
class FullyConnectedLayer(torch.nn.Module):
    def __init__(
        ...
        self.bias = (
            torch.nn.Parameter(torch.from_numpy(bias_init / lr_multiplier)) if bias else None
        )
...
class SynthesisInput(torch.nn.Module):
    ...
    def forward(self, w):
        ...
        amplitudes = (
            1 - (freqs.norm(dim=2) - self.bandwidth) / (self.sampling_rate / 2 - self.bandwidth)
        ).clamp(0, 1)
...
class SynthesisLayer(torch.nn.Module):
    def __init__(
        ...
        self.up_taps = (
            filter_size * self.up_factor if self.up_factor > 1 and not self.is_torgb else 1
        )
        self.register_buffer(
            "up_filter",
        ...
        self.down_taps = (
            filter_size * self.down_factor if self.down_factor > 1 and not self.is_torgb else 1
        )
