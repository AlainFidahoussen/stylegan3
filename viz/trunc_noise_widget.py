    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            with imgui_utils.item_width(viz.font_size * 10), imgui_utils.grayed_out(num_ws == 0):
                _changed, self.trunc_psi = imgui.slider_float(
                    "##psi", self.trunc_psi, -1, 2, format="Psi %.2f"
                )
            imgui.same_line()
            if num_ws == 0:
                imgui_utils.button(
                    "Cutoff 0", width=(viz.font_size * 8 + viz.spacing), enabled=False
                )
            else:
                with imgui_utils.item_width(viz.font_size * 8 + viz.spacing):
                    changed, new_cutoff = imgui.slider_int(
                        "##cutoff", self.trunc_cutoff, 0, num_ws, format="Cutoff %d"
                    )
                    if changed:
            ...
            with imgui_utils.grayed_out(not has_noise):
                ...
                with imgui_utils.grayed_out(not self.noise_enable):
                    with imgui_utils.item_width(
                        -1 - viz.button_w - viz.spacing - viz.font_size * 4
                    ):
            ...
            with imgui_utils.grayed_out(is_def_trunc and not has_noise):
                ...
                if imgui_utils.button(
                    "Reset", width=-1, enabled=(not is_def_trunc or not is_def_noise)
                ):
                    self.prev_num_ws = num_ws
                    self.trunc_psi = 1
                    self.trunc_cutoff = num_ws
        ...
        if self.noise_anim:
            self.noise_seed += 1
        viz.args.update(
            trunc_psi=self.trunc_psi, trunc_cutoff=self.trunc_cutoff, random_seed=self.noise_seed
        )
        viz.args.noise_mode = (
            "none" if not self.noise_enable else "const" if self.noise_seed == 0 else "random"
        )
