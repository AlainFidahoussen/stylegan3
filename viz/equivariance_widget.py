...
class EquivarianceWidget:
    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            _clicked, dragging, dx, dy = imgui_utils.drag_button(
                "Drag fast##xlate", width=viz.button_w
            )
            if dragging:
            ...
            imgui.same_line()
            _clicked, dragging, dx, dy = imgui_utils.drag_button(
                "Drag slow##xlate", width=viz.button_w
            )
            if dragging:
            ...
            imgui.same_line()
            with (
                imgui_utils.item_width(-1 - viz.button_w - viz.spacing),
                imgui_utils.grayed_out(not self.xlate.anim),
            ):
            ...
            if imgui_utils.button(
                "Reset##xlate", width=-1, enabled=(self.xlate != self.xlate_def)
            ):
        ...
        if show:
            ...
            imgui.same_line(viz.label_w)
            with imgui_utils.item_width(viz.font_size * 8):
                _changed, self.rotate.val = imgui.input_float(
                    "##rotate", self.rotate.val, format="%.4f"
                )
            imgui.same_line(viz.label_w + viz.font_size * 8 + viz.spacing)
            _clicked, dragging, dx, _dy = imgui_utils.drag_button(
                "Drag fast##rotate", width=viz.button_w
            )
            if dragging:
                self.rotate.val += dx / viz.font_size * 2e-2
            imgui.same_line()
            _clicked, dragging, dx, _dy = imgui_utils.drag_button(
                "Drag slow##rotate", width=viz.button_w
            )
            if dragging:
            ...
            with (
                imgui_utils.item_width(-1 - viz.button_w - viz.spacing),
                imgui_utils.grayed_out(not self.rotate.anim),
            ):
                changed, speed = imgui.slider_float(
            ...
            imgui.same_line()
            if imgui_utils.button(
                "Reset##rotate", width=-1, enabled=(self.rotate != self.rotate_def)
            ):
                self.rotate = dnnlib.EasyDict(self.rotate_def)

        if show:
            imgui.set_cursor_pos_x(
                imgui.get_content_region_max()[0] - 1 - viz.button_w * 1 - viz.font_size * 16
            )
