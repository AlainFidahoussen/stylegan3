    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            with imgui_utils.grayed_out(not self.fft_show):
                if imgui_utils.button(
                    "Reset##fft_flags", width=-1, enabled=(self.fft_show or not self.fft_all)
                ):
                    self.fft_show = False
                    self.fft_all = True
            ...
            with imgui_utils.grayed_out(not self.fft_show):
                ...
                imgui.same_line()
                if imgui_utils.button(
                    "Reset##fft_range_db", width=-1, enabled=(self.fft_range_db != 50)
                ):
                    self.fft_range_db = 50
        ...
        self.base_channel = min(max(self.base_channel, 0), base_channel_max)
        viz.args.layer_name = (
            self.cur_layer if len(layers) > 0 and self.cur_layer != layers[-1].name else None
        )
        viz.args.update(
        ...
        if self.fft_show:
            viz.args.update(
                fft_all=self.fft_all, fft_range_db=self.fft_range_db, fft_beta=self.fft_beta
            )
